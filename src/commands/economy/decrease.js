...
module.exports = async (client, config) => {
  client.on("interactionCreate", async (interaction) => {
    if (interaction.isCommand() && interaction.commandName == "decrease") {
      ...
      try {
        ...
        const levelsToDecrease = interaction.options.getInteger("level");
        ...
        // Subtract levels from the user
        userBalance.level -= levelsToDecrease;

        // Deduct 100 ice coins per level
        userBalance.iceCoins -= levelsToDecrease * 500;
        ...
        await targetUser.send({
          embeds: [
            new MessageEmbed()
              .setColor(color.gray)
              .setTitle(`${emojis.levelDown} Level Down ${userBalance.level}!`)
              .setDescription(
                `${emojis.threadMarkmid} ${interaction.user} decreased your level with ${levelsToDecrease} levels your new level is **LV${userBalance.level}**\n${emojis.threadMark} Your new balance is ${emojis.ic} **${formattedBalance} Ice Coins**`,
              ),
          ],
        });
        ...
        await interaction.editReply({
          embeds: [
            new MessageEmbed()
              .setColor(color.gray)
              .setTitle(`${emojis.check} Levels decreased Successfully!`)
              .setDescription(
                `${emojis.threadMarkmid} decreased ${targetUser}'s level with **${levelsToDecrease}** levels, his new level is **Lv${userBalance.level}**\n${emojis.threadMark} And his new balance is: ${emojis.ic} **${formattedBalance} Ice Coins**`,
              ),
          ],
        });
        ...
        console.log(
          ...
          `\x1b[31m with`,
          `\x1b[35m ${levelsToDecrease} levels`,
        );
      } catch (error) {
        console.error(
          `\x1b[0m`,
          `\x1b[33m 〢`,
          `\x1b[33m ${moment(Date.now()).format("LT")}`,
          `\x1b[31m Error in decreasing level:`,
        ...
        return interaction.editReply({
          embeds: [
            new MessageEmbed()
              ...
              .setDescription(
                `${emojis.threadMark} Something went wrong while decreasing ${targetUser}'s level. Please try again later.`,
              ),
          ],
          ephemeral: true,
          components: [],
